							...
							<div>
								...
								<input
									...
									onChange={e =>
										setForm({
											...form,
											oldPrice: parseInt(e.target.value) || 0,
										})
									}
                  ...
                  <label htmlFor="hidePrice" className="text-sm font-bold text-gray-700 cursor-pointer select-none flex items-center gap-1.5">
					...
					<div className='bg-white rounded-xl shadow-sm p-6 border border-gray-200'>
						...
						<div className='flex flex-wrap gap-2'>
							{properties.map(prop => {
								const propertyIds = Array.isArray(form.propertyIds)
									? form.propertyIds
									: []
								const selected = prop.id ? propertyIds.includes(prop.id) : false

								const label =
									(prop[locale as keyof typeof prop] as LocalizedContent)
										?.label ||
									prop.ru?.label ||
									prop.key
								...
								return (
									<button
										...
										onClick={() => {
											setForm(prev => {
												const currentIds = Array.isArray(prev.propertyIds)
													? prev.propertyIds
													: []
					...
					<div className='bg-white rounded-xl shadow-sm p-6 border border-gray-200'>
						...
						<div className='space-y-8'>
							{(['ru', 'en', 'uz', 'tj'] as const).map(loc => (
								<div
									key={loc}
									className='relative border border-gray-100 rounded-xl p-5 bg-gray-50/30'
								>
									...
									<div className='space-y-4'>
										<div>
											...
											<input
												...
												value={form[loc].name}
												onChange={e =>
													updateLocale(loc, 'name', e.target.value)
												}
												className='w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none bg-white'
											/>
										</div>
										<div>
											...
											<textarea
												...
												onChange={e =>
													updateLocale(loc, 'description', e.target.value)
												}
												rows={4}
